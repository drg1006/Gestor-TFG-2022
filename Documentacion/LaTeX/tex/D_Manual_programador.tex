...
\begin{itemize}
	...
	\item \textbf{Creación de informes}: se ha realizado un informe para el área \emph{Lenguajes y Sistemas informáticos}, para un total de 10 alumnos, solo participarían dos proyectos de la pantalla de activos, ya que son los que tienen fecha de asignación de este último año. Este informe generado se encuentra en  \texttt{Documentacion/Pruebas} con el nombre \texttt{InformeP01} y se puede ver su resultado en la imagen \ref{fig:P01Informe}.
	
	\item \textbf{Proponer TFG}: el último proyecto del este archivo tiene como título \emph{GII 21.04 ....}, por lo que el título de este se debería de modificar y al estar en el curso 2022/2023 debería ser 22.01 como se observa en la imagen \ref{fig:P01SubirTFG}. Una vez indicamos los campos obligatorios se añade el proyecto y podemos ver que su estado es pendiente en la pantalla de \emph{Administrar TFGs} como se ve en la imagen \ref{fig:P01AdminTFGSubido}.	
	
	\item \textbf{Aceptar/Denegar TFG}: en este apartado aceptaremos el proyecto que hemos añadido anteriormente y que modificará su estado, que ahora es \emph{vacio}, si lo hubiésemos denegado habría cambiado a \emph{denegado}, como se ve en la imagen \ref{fig:P01AceptTFG}.
	
	\item \textbf{Modificar TFG}: en esta prueba primero cambiaremos la información del proyecto añadido anteriormente, para ello en la pantalla de administración de proyectos, lo seleccionamos y \emph{clickamos} en el botón \emph{Modificar}.
		\begin{itemize}
				\item \textbf{Modificar datos y mantener activo}: primero pulsaremos en el botón \emph{No} que se ve en la imagen \ref{fig:P01No}. Posteriormente cambiaremos los datos a los que se ve en la imagen \ref{fig:P01Abierto} y \emph{clickaremos} en el botón \emph{Aceptar cambios y dejar abierto} que se nos habrá habilitado. Podemos ver el resultado en la pantalla de activos o en la de modificación de proyectos actualizado. Ver imagen \ref{fig:P01AbiertoActualizado}.
				
				\item \textbf{Modificar datos y cerrarlo}: por último cogeremos este proyecto, \emph{clickaremos} en la opción \emph{Sí} que se ve en la imagen \ref{fig:P01No} y añadiremos toda la información necesaria, como se ve en la imagen \ref{fig:P01Cerrado} para cerrarlo y moverlo a histórico tras pulsar en \emph{Aceptar cambios y mover a histórico}
		\end{itemize}
...
\imagenflotante{P01Informe}{Comprobación creación informe.}{0.9}
\imagenflotante{P01SubirTFG}{Comprobación campos del propuesta de proyecto.}{0.9}
\imagenflotante{P01AdminTFGSubido}{Comprobación del TFG subido.}{0.9}
\imagenflotante{P01AceptTFG}{Comprobación cambio de estado.}{0.9}
\imagenflotante{P01No}{Clickar en la opción 'No'.}{0.9}
\imagenflotante{P01Abierto}{Nuevos datos a introducir.}{0.9}
\imagenflotante{P01AbiertoActualizado}{Información ya actualizada.}{0.9}
\imagenflotante{P01Cerrado}{Nuevos datos a introducir para cerrar un proyecto.}{0.9}
