...
public class SistInfDataXls extends SistInfDataAbstract implements Serializable {
     ...
     *                   Nombre de la tabla.
     * @param minimo
     *                   True si queremos el curso mínimo, false si queremos el
     *                   curso
     *                   máximo.
    ...
    public List<Integer> getRankingTotal() {
        ...
        try {
            ...
            if (scoreList.size() > 0) {
                ...
                // Pasamos la lista a un stream que tiene el método distinct (elimina
                // duplicados)
     ...
     *                   Nombre de la columna.
     * @param tableName
     *                   nombre de la hoja del xls o del csv donde se encuentran los
     *                   datos
    ...
    protected List<String> addYearsCurseToList() {
        ...
        try {
            ...
            while (result_Curse.next()) {
                ...
                // curses.add(dateIni.toString().substring(6) +
                // dateEnd.toString().substring(5));
            ...
            for (i = 0; i < datesCurse.size(); i++) {
                for (j = 0; j < datesCurse.size(); j++) {
                    if (i != j) {
                        ...
                        if (datesCurse.get(i).equals(datesCurse.get(j))
                                || (dateInic.equals(datesCurse.get(j).substring(5))
    ...
    public List<String> getProfesoresDeDepartamento(String departamento) {
        ...
        try {
            Recordset result = connection.executeQuery(sql);
            while (result.next()) {
                profesoresDeDepartamento.add(result.getField(NOMBRE_APELLIDOS).toString());
            }
        } catch (FilloException ex) {
