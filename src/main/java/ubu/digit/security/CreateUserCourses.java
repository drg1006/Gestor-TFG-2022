...
public class CreateUserCourses {
     ...
     * devuelve la lista con los cursos obtenidos de ese JSON ( en el metodo
     * createCourses)
     * 
     * @param userid
    ...
    /**
     * Obtiene a partir del JSONArray con los cursos un JSONObject donde figuran los
     ...
     * Y, por tanto este usuario tendrá permisos de actualización también en la app
     * Gestor-TFG-2021.
     * En caso contrario, devolverá false y no podrá acceder a la vista de
     * UploadView.
     * 
     * @param jsonArray
     * @param course
    ...
    public Boolean findPermission(JSONArray jsonArray, Course course, String permission) {
        for (int i = 0; i < jsonArray.length(); ++i) {
            ...
            for (int j = 0; j < options.length(); ++j) {
                ...
                if (permission.equals(option.getString(Constants.NAME))
                        && option.getBoolean(Constants.AVAILABLE) == true) {
        ...
        LoginView.createErrorLogin("El usuario no tiene los permisos requeridos para acceder",
                "Acceda con un usuario con permisos de actualización");
        return false;
    }
}
